...
"""
Данный БД - копия БД для курсовой работы, с одним отличием, здесь для тестов - локальная БД.

"""
with psycopg2.connect(database="bd_VKinder", user="postgres", password="**********") as conn:
    ...
    def update_city_delete_favorites(conn, check: int, city_new: str):
        """
        ...
        with conn.cursor() as cur:
            ...
            conn.commit()
            print("город поиска изменен. Клиенты прежнего города удалены")
            return True
